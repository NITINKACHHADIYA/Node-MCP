...
function applyRule(s: JsonSchema, name: string, c: unknown[] = []): void {
  switch (name) {
    case 'isString': s.type = 'string'; break;
    case 'isNumber': case 'isNumberString': s.type = name === 'isNumber' ? 'number' : 'string'; break;
    case 'isInt': s.type = 'integer'; break;
    case 'isBoolean': s.type = 'boolean'; break;
    case 'isDate': case 'isDateString': case 'isISO8601': s.type = 'string'; s.format = 'date-time'; break;
    case 'isEmail': s.type = 'string'; s.format = 'email'; break;
    case 'isUrl': s.type = 'string'; s.format = 'uri'; break;
    case 'isUUID': s.type = 'string'; s.format = 'uuid'; break;
    case 'isPositive': s.exclusiveMinimum = 0; break;
    case 'isNegative': s.exclusiveMaximum = 0; break;
    case 'min': s.minimum = c[0]; break;
    case 'max': s.maximum = c[0]; break;
    case 'minLength': s.minLength = c[0]; break;
    case 'maxLength': s.maxLength = c[0]; break;
    case 'length': s.minLength = c[0]; if (c[1] !== undefined) s.maxLength = c[1]; break;
    case 'matches': if (c[0] instanceof RegExp) s.pattern = c[0].source; break;
    case 'isIn': s.enum = c[0] as unknown[]; break;
    case 'isEnum': s.enum = (c[1] as unknown[]) ?? Object.values(c[0] as object); break;
    case 'isNotEmpty': if (s.type === 'string') s.minLength = Math.max(1, Number(s.minLength ?? 0)); break;
    case 'isArray': s.type = 'array'; break;
    case 'arrayMinSize': s.minItems = c[0]; break;
    case 'arrayMaxSize': s.maxItems = c[0]; break;
    case 'isObject': s.type = 'object'; break;
  }
}
...
function initial(dto: Function, prop: string): JsonSchema {
  ...
  const api = Reflect.getMetadata?.('swagger/apiModelProperties', dto.prototype, prop) as
    | { description?: string; example?: unknown; enum?: unknown }
    | undefined;
